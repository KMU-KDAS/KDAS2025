...
이 고민 끝에, 우리는 자율주행 연구에서 널리 쓰이는 개념인 **Waypoint(경유점)** 에 도달했다.  
...
우리는 차선이라는 시각적 정보에서  
차량이 따라갈 수 있는 좌표 기반의 구조적 표현을 만들고자 했다.  
...
차량이 따라갈 수 있는 ‘안정적 경로’를 표현하는 하나의 구조적 언어다.  
이 언어를 제대로 사용하지 않으면  
...
Waypoints를 통해  
“차선 → 경로 → 제어 입력”이라는 흐름이 만들어지자  
자연스럽게 다음 단계가 보이기 시작했다.
...
우리가 구축하게 되는 전체 Path Planning 구조의 출발점이 되었다.
