...
하지만 차량을 실제로 움직이는 제어기는 이미지가 아니라 다음과 같은 **기하학적 목표**를 필요로 한다.
...
우리가 만들려고 했던 것은  
차선이라는 시각적 정보를  
**차량이 따라갈 수 있는 좌표 기반의 구조적 표현으로 변환하는 과정**이었다.

Waypoint는 단순한 위치 정보가 아니라 다음과 같은 역할을 한다.

- 차선의 흐름을 **수학적으로 표현**하고  
- 차량의 주행 방향을 **자연스럽게 이어주며**  
- 제어기가 사용할 수 있는 **경로의 언어(Path Language)** 가 된다  
...
차선을 본다는 건 **인지의 시작**일 뿐, 그것만으로는 주행을 만들 수 없다.

차량이 실제로 움직이려면 다음이 필요하다.

- 픽셀 기반 인지를 **실제 거리 기반 좌표로 변환**하고  
- 그 좌표가 **연속적(continuous)** 이어야 하며  
- 센서 노이즈에도 흔들리지 않는 **안정적 경로 구조**로 표현돼야 한다  
...
**우리가 구축한 전체 Path Planning 구조의 출발점**이 되었다.
