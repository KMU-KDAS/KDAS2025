...
2D 픽셀 공간에서 보이는 곡률은 그저 *화면 위의 곡선*일 뿐,  
차량 기준 실제 거리 단위(m)에서 정의되는 **물리적 곡률(1/m)**과는 전혀 다르다.  
따라서 2D 정보만으로는 Stanley 제어기가 요구하는  
횡오차·헤딩오차·곡률을 직접 계산할 수 없다.

이 한계를 해결하기 위해 Depth 카메라를 결합했다.  
Depth가 제공하는 각 픽셀의 실제 거리 정보를 이용해 SCNN 차선 픽셀을  
실세계 좌표계의 3차원 점으로 변환함으로써,  
단순한 이미지 경계선이 **실제 도로의 3D 경로**로 재해석되도록 만들었다.
